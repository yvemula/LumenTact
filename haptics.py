...
import time
from .hardware import GLOBAL_CONTROLLER  # <-- IMPORT THE MOCK HARDWARE
...
def process_frame_detections(all_detections):
    ...
    if not all_detections:
        # Optionally, send an "all-clear" signal or just do nothing
    ...
    print(
        f"[HAPTIC] Frame analysis complete. Most critical: '{class_name}' (Urgency: {most_critical['urgency']:.2f})."
    )

    # --- UPDATED SECTION ---
    # Instead of just printing, send to the hardware controller
    GLOBAL_CONTROLLER.send_feedback(signal_type, intensity, direction)
    # --- END UPDATED SECTION ---
